...
function setDate() {
	var t = [],
	    // Начальный массив лет
	l = -1,
	    // Счетчик лет
	r = -1,
	    // Счетчик месяцев
	y,
	    // Текущий год в массиве
...
function setOffsetArr() {
	var date = setDate(),
	    ...
	    // Высота шапки (она переменная)
	gridHeight = document.querySelector('.portfolio-grid').offsetHeight,
	    // Высота всего блока с элементами портфолио
	offsetArr = [];

	date.forEach(function (item, i, arr) {
		var itemTop, offsetArrItem;
  ...
  */
		...
		if (itemTop >= document.querySelector('.scale').offsetHeight - 20) {
   ...
   */
	...
	for (var i = 1; i < offsetArr.length; i++) {
		// Проверяем массив на наличие некорректных данных с начала
	...
	for (var i = offsetArr.length - 1; i > 0; i--) {
		// Проверяем массив на наличие некорректных данных с конца
		if (offsetArr[i].offset < offsetArr[i - 1].offset + 20 && offsetArr[i].last) {
...
function setScale() {
	...
	elM,
	    // Маркер месяцев
	date = setDate(),
	    // Массив с годами и месяцами
	offsetArr = setOffsetArr(),
	    // Массив с отступами
	h,
	    headerH = document.querySelector('.header').offsetHeight,
	    // Высота шапки (она переменная)
	...
	scaleBlock.onmousedown = function (t) {
		...
		$('html, body').stop();
		$('html, body').animate({
			scrollTop: h * (gridHeight - document.documentElement.clientHeight + headerH) + document.querySelector('.portfolio-grid').getBoundingClientRect().top + pageYOffset - headerH
		}, 500);
		...
		document.onmousemove = function (target) {
			...
			scroll = h * (gridHeight - document.documentElement.clientHeight + headerH) + document.querySelector('.portfolio-grid').getBoundingClientRect().top + pageYOffset - headerH;
			//			
			//			$('html, body').animate({
			//				scrollTop: scroll
			//			}, 300);
	...
	var c = 0,
	    // Счетчик количества табличек, ушедших за пределы блока
	timeLineYear = [],
	    // Весь массив с значениями отступа от верха
	index = 0;
...
$(document).ready(function () {
	...
	if ($.fn.owlCarousel) {
		...
		$('.first__container').owlCarousel((_$$owlCarousel = {
			loop: true,
			margin: 10,
			nav: false,
			items: 1
		}, _defineProperty(_$$owlCarousel, 'margin', 0), _defineProperty(_$$owlCarousel, 'center', true), _$$owlCarousel));

		$('.clients__container').owlCarousel((_$$owlCarousel2 = {
			loop: true,
			margin: 10,
			nav: false,
			//    autoWidth:true,
			...
			autoWidth: true,
			stagePadding: 150
		}, _defineProperty(_$$owlCarousel2, 'margin', 50), _defineProperty(_$$owlCarousel2, 'autoplay', true), _defineProperty(_$$owlCarousel2, 'autoplayTimeout', 1000), _defineProperty(_$$owlCarousel2, 'responsive', {
			0: {
				items: 1,
				stagePadding: 70
			},
